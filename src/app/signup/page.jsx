...
function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user.userName.length > 0 && user.password.length > 0) {
     ...
     setAlert("enter all credentials!");
    }
  };
  return (
    <div className=" bg_url flex flex-col items-center justify-center min-h-screen py-2">
        { alert != "" &&<div role="alert" className=" absolute z-[100] alert alert-warning">
  <svg
  ...
  <span>{alert}</span>
      ...
      <h1 className="pacifico-regular mb-6">_signup_</h1>
      {!loading && (
        <>
          <form
            onSubmit={handleSubmit}
            className="flex flex-row bg-slate-100 bg-opacity-70 border-[2px]  px-12 w-[40vw] p-5 rounded-2xl border-teal-950  content-between  "
          >
           <div className="" >
           <div>
              <input
            ...
            <input
              className="mt-2  text-white p-1 w-full rounded-2xl fontfamily hover:bg-teal-800 btn-success bg-teal-950 "
              type="submit"
              value=" Signup"
            />
           </div>
